...
class FinancialAnalysis:
    def __init__(self, url, token, strategy, market, report, conditions,
                 number_of_bars, footer, show_indicators, theme, symbol, dpi):
        self.log = args.log
        self.url = url
        self.token = token
    ...
    def prepare_fast_chart(self):
        ...
        if self.log:
            self.call_log('Request for chart data:')
            self.call_log(self.queryChartData_without_indicators % (self.strategy, self.market, self.number_of_bars))
        ...
        if self.log:
            ...
            json.dump(data, open(self.currentDateTime + "_chart_data.js", "w"))

        assert data, "Unexpected response: {}".format(data)
        market_indicators = data.get("marketIndicators")
        assert market_indicators, "No marketIndicators field found in {}".format(data)
    ...
    def prepare_normal_chart(self):
        ...
        if self.log:
            ...
            json.dump(data, open(self.currentDateTime + "_chart_config.js", "w"))

        market_name = data['market']['symbol']
        assert data, "Unexpected response: {}".format(data)

        chart_config = data.get("chartConfig")
        assert chart_config, "No chartConfig field found in: {}".format(data)
        ...
        response_chart_data = requests.post(
            self.url,
            headers={'Accept': 'application/json', 'Authorization': 'Bearer {}'.format(self.token)},
            data={"query": self.queryChartData_with_indicators % (self.strategy, self.market, self.number_of_bars, ",".join(indicator_names))})
        ...
        if self.log:
            self.call_log('Request for chart data:')
            self.call_log(self.queryChartData_with_indicators % (self.strategy, self.market, self.number_of_bars, ",".join(indicator_names)))
    ...
    def declare_variables(self):
        self.log_file_name = datetime.now().strftime('%Y_%m_%d_%H_%M_%S')+'_logs.txt'
...
if __name__ == '__main__':
    ...
    # main routine
    fin_obj = FinancialAnalysis(_url, _token, _strategy, _market, _report, _conditions, _number_of_bars, _footer, _show_indicators, _theme, _symbol, _dpi)
    fin_obj.process()
    b=datetime.now()
