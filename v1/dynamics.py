...
def render_panel(axes, settings, data, default):
    ...
    for plot in plots:
        ...
        if plots[plot]["type"] == "line":
            if color:
                axes.plot(xs, ys, linestyle=linestyle, label=plot, color=color)
            else:
                axes.plot(xs, ys, linestyle=linestyle, label=plot)
...
def get_data(url, token, strategy, market, report, conditions, numberOfBars, footer):
    ...
    candlestick_ochl(default, candlesticks, width=0.5, colorup='g', colordown='r', alpha=0.6)
